...
Const c_intFieldsCount As Integer = 1
...
Private Sub Form_Load()
    ...
    '
     ...
     strSQL = "select CXDX_ID from " & strstep & " where BZZLX = 3 and LCYJ_ID = "
      strSQL = strSQL & LCYJIDtemp
      Set recEdited = OraDatabase.CreateDynaset(strSQL, 0&)
      strstep = prefixname & "LCZZ"
      strSQL = "select ZZ_ID,ZZMC from " & strstep
      ...
      While Not recEdited.EOF
            ...
            strCondition = strCondition & " ZZ_ID = " & recEdited.Fields("CXDX_ID").Value
      ...
      strSQL = strSQL & " order by ZZ_ID asc"
      
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
      Else
      
      strSQL = "select ZZ_ID,ZZMC from " & strstep & " where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZZ_ID as 症状ID, ZZMC as 症状名称 from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZZ_ID as 症状ID, ZZMC as 症状名称 from " & strstep & strCondition
    End If
    ...
    FrmQUERYD.Show vbModal
     ...
     strSQL = "select ZZ_ID,ZZMC from " & strstep
     Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
      recFields.FindFirst ("ZZ_ID=" & FrmQUERYD.intID)
      intState = fsBrowse
      blnNoRecorder = False
      Call SetButtons(intState)
