...
Private Sub Form_Load()
    ...
    '
     ...
     strSQL = "select JTZlGC_ID from " & strstep & " where ZLLX = 5 and LCYJ_ID = "
      strSQL = strSQL & LCYJIDtemp
      Set recEdited = OraDatabase.CreateDynaset(strSQL, 0&)
      
      strstep = prefixname & "QTLF"
      strSQL = "select ZLGC_ID,FFMC,FFMS,ZZ from " & strstep
      ...
      strSQL = "select ZLGC_ID,FFMC,FFMS,ZZ from " & strstep & "  where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, FFMC as 方法名称 from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, FFMC as 方法名称 from " & strstep & strCondition
    End If
