...
Private Sub Form_Load()
    ...
    '
      ...
      strSQL = "select JTZlGC_ID from " & strstep & " where ZLLX = 3 and LCYJ_ID = "
      strSQL = strSQL & LCYJIDtemp
      Set recEdited = OraDatabase.CreateDynaset(strSQL, 0&)
      
      strstep = prefixname & "AMLF"
      strSQL = "select ZLGC_ID,AMFF,AMSF,AMSJ,AMCS,JL,ZZ from " & strstep
      ...
      strSQL = "select ZLGC_ID,AMFF,AMSF,AMSJ,AMCS,JL,ZZ from " & strstep & " where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, AMFF as 针灸方法 from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, AMFF as 针灸方法 from " & strstep & strCondition
    End If
