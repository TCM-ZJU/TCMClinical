...
Private Sub Form_Load()
    ...
    '
     ...
     strSQL = "select JTZlGC_ID from " & strstep & " where ZLLX = 4 and LCYJ_ID = "
      strSQL = strSQL & LCYJIDtemp
      Set recEdited = OraDatabase.CreateDynaset(strSQL, 0&)
      strstep = prefixname & "XYLF"
      strSQL = "select ZLGC_ID,GYFS,YWMC,YWJX,GYSJ,ZLYZ from " & strstep
      ...
      strSQL = "select ZLGC_ID,GYFS,YWMC,YWJX,GYSJ,ZLYZ from " & strstep & " where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, GYFS as 给药方式 from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID, GYFS as 给药方式 from " & strstep & strCondition
    End If
