...
Private Sub Form_Load()
    ...
    '
     ...
     strSQL = "select CXDX_ID from " & strstep & " where BZZLX = 2 and LCYJ_ID = "
      strSQL = strSQL & LCYJIDtemp
      Set recEdited = OraDatabase.CreateDynaset(strSQL, 0&)
      strstep = prefixname & "ZH"
      strSQL = "select ZH_ID,ZHMC from " & strstep
      ...
      While Not recEdited.EOF
            ...
            strCondition = strCondition & " ZH_ID = " & recEdited.Fields("CXDX_ID").Value
      ...
      strSQL = strSQL & " order by ZH_ID asc"
      
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
      Else
      
      strSQL = " select ZH_ID,ZHMC from " & strstep & " where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZH_ID as 证候ID, ZHMC as 证候名称 from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZH_ID as 证候ID, ZHMC as 证候名称 from " & strstep & strCondition
    End If
    ...
    FrmQUERYD.Show vbModal
     ...
     strSQL = "select ZH_ID,ZHMC from " & strstep
     Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
      recFields.FindFirst ("ZH_ID=" & FrmQUERYD.intID)
      intState = fsBrowse
      blnNoRecorder = False
      Call SetButtons(intState)
