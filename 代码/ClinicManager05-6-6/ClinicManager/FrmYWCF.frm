...
Private Sub Form_Load()
    ...
    On Error GoTo PROC_ERR
     ...
     strSQL = "select JTZlGC_ID from " & strstep & " where  LCYJ_ID = "
      ...
      strstep = prefixname & "YWCF"
      strSQL = "select ZLGC_ID,YWCF_ID,YWCFMC,YWCFJL,JLDW from " & strstep
      ...
      strSQL = "select ZLGC_ID,YWCF_ID,YWCFMC,YWCFJL,JLDW from " & strstep & "  where 1<>1"
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID,YWCF_ID as 药物成分ID from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select ZLGC_ID as 治疗过程ID,YWCF_ID as 药物成分ID from " & strstep & strCondition
    End If
