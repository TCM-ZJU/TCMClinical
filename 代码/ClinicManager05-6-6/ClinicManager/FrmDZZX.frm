...
Const c_intFieldsCount As Integer = 6
...
Private Sub Form_Load()
    ...
    '
    strstep = prefixname & "DZZ"
    If LCYJIDtemp = -1 Then
      strSQL = "select DZZ_ID,LCYJ_ID,DZZMC,NL,LS,ZLYW,JC from " & strstep & " where LCYJ_ID in ( select LCYJ_ID from " & prefixname & "LCYJ" & suffixCondition & " ) "
   Else
...
Private Sub cmdQuery_Click()
    ...
    If txtCondition.Text = "" Then
        FrmQUERYD.objOO4O.RecordSource = "select DZZ_ID as 对照组ID, LCYJ_ID as 临床研究ID from " & strstep & " where 1=1"
    Else
        strCondition = txtCondition.Text
        FrmQUERYD.objOO4O.RecordSource = "select DZZ_ID as 对照组ID, LCYJ_ID as 临床研究ID from " & strstep & strCondition
    End If
    ...
    FrmQUERYD.Show vbModal
     ...
     strSQL = "select DZZ_ID,LCYJ_ID,DZZMC,NL,LS,ZLYW,JC from " & strstep
      recFields.FindFirst ("DZZ_ID=" & FrmQUERYD.intID)
      Set recFields = OraDatabase.CreateDynaset(strSQL, 0&)
